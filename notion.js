  ...
  try {
    ...
    if (action === 'findLead') {
      ...
      return {
        ...
        body: JSON.stringify({ result: res.data.results?.[0] || null })
      };
    }
    ...
    if (action === 'upsertLead') {
      ...
      if (!existing) {
        props['Date inscription'] = { date: { start: new Date().toISOString().split('T')[0] } };
      }
    ...
    if (action === 'saveVersement') {
      ...
      const findRes = await notionRequest('POST', `/v1/databases/${dbId}/query`, {
        filter: {
          and: [
            ...
            { property: 'Mois déclaré', rich_text: { equals: moisKey } }
          ]
        }
      });
      const existing = findRes.data.results?.[0];
      ...
      const props = {
        ...
        'Date déclaration':     { date: { start: dateStr } },
      };
      ...
      // Mettre à jour le capital dans la fiche lead
      if (leadPageId) {
        await notionRequest('PATCH', `/v1/pages/${leadPageId}`, {
          properties: { 'Capital départ': { number: capitalTotal } }
        });
      }
